...
class App extends React.Component {
	...
	setSearch = (event, subUser) => {
		...
		this.setState({search: subUser, searchable:true}, ()=>this.confirmSearch())	
		}
		if(event.key==='Enter' & this.state.search!==this.state.user.name){
			if(this.state.user.name===undefined){
				this.confirmSearch();	
			} else if(this.state.search.toUpperCase()!==this.state.user.name.toUpperCase()){
				this.confirmSearch();
			} 
		}
	}
		...
		if(this.state.searchable===true){		
			...
			.then(data=>{
				if(data.name===undefined && data.url===undefined){
					this.setState({displayUser:undefined})
				} else {
					this.setUser(data)
				}
			})
